...
func getHandType(cards: [Substring]) -> Int {
    ...
    switch(cardRepeats.count) {
        case 0, 1:
            return 7
        case 2:
            if (cardRepeats.sorted(by: {m, n in m.value > n.value}).first!.value) > 3 - amountOfJokers {
                return 6
            }
            return 5
        case 3:
            if (cardRepeats.sorted(by: {m, n in m.value > n.value}).first!.value) > 2 - amountOfJokers {
                return 4
            }
            return 3
        case 4:
...
while let input = readLine() {
    ...
    for i in 1...handCards.count {
        switch(handCards[i-1]) {
            ...
            case "J":
                handValue += Array(repeating: 100, count: (handCards.count - i)).reduce(1,*)
            case "T":
